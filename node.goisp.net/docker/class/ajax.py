...
# | 宝塔Linux面板
# +-------------------------------------------------------------------
# | Copyright (c) 2015-2016 Pagoda software(http://goisp.net) All rights reserved.
# +-------------------------------------------------------------------
# | Author: hwliang <hwl@goisp.net>
# +-------------------------------------------------------------------
from gopanel import session,request
...
class ajax:
    ...
    def get_beta_logs(self,get):
        try:
            # data = json.loads(public.HttpGet('https://console.goisp.net/api/panel/get_beta_logs_en'))
    ...
    def _get_cloud_phplib(self):
        if not session.get('download_url'): session['download_url'] = 'http://node.goisp.net'
