    ...
    def crate_let_by_oper(self,data):
        ...
        try:
            ...
            if not 'renew' in data:
                ...
                gopanel.dns_client = sewer.Client(domain_name = data['first_domain'],dns_class = None,account_key = data['account_key'],domain_alt_names = data['domains'],contact_email = str(data['email']) ,ACME_AUTH_STATUS_WAIT_PERIOD = 15,ACME_AUTH_STATUS_MAX_CHECKS = 5,ACME_REQUEST_TIMEOUT = 20,ACME_DIRECTORY_URL = self.let_url)
                domain_dns_value = "placeholder"
                dns_names_to_delete = []
                self.write_log(public.getMsg("REGISTER_ACCOUNT"))
                gopanel.dns_client.acme_register()
                authorizations, finalize_url = gopanel.dns_client.apply_for_cert_issuance()
                responders = []
                self.write_log(public.getMsg("GET_VERIFICATION_INFO"))
                for url in authorizations:
                    identifier_auth = gopanel.dns_client.get_identifier_authorization(url)
                    ...
                    acme_keyauthorization, domain_dns_value = gopanel.dns_client.get_keyauthorization(dns_token)
                ...
                for i in responders:  
                    ...
                    auth_status_response = gopanel.dns_client.check_authorization_status(i["authorization_url"])
                    if auth_status_response.json()["status"] == "pending":
                        gopanel.dns_client.respond_to_challenge(i["acme_keyauthorization"], i["dns_challenge_url"])

                for i in responders:
                    self.write_log(public.getMsg("GET_CA_V_RES",(i['dns_name'],)))
                    gopanel.dns_client.check_authorization_status(i["authorization_url"], ["valid","invalid"])
                self.write_log(public.getMsg("ALL_DOMAIN_V_PASS"))
                certificate_url = gopanel.dns_client.send_csr(finalize_url)
                self.write_log(public.getMsg("GET_CERT_CONTENT"))
                certificate = gopanel.dns_client.download_certificate(certificate_url)

                if certificate:
                    certificate = self.split_ca_data(certificate)
                    result['cert'] = certificate['cert']
                    result['ca_data'] = certificate['ca_data']
                    result['key'] = gopanel.dns_client.certificate_key
                    result['account_key'] = gopanel.dns_client.account_key
                    result['status'] = True
                    gopanel.dns_client = None
                else:
