...
try:
    from gopanel import session,admin_path_checks,g,request,cache
    import send_mail
except:pass
class config:
 ...
 # 取Session缓存方式
    ...
    def get_login_log(self,get):
        ...
        import page
        page = page.Page()
        count = public.M('logs2').where('type=?', (u'gopanel login reminder',)).field('log,addtime').count()
        limit = 7
        info = {}
        info['count'] = count
        info['row'] = limit
        ...
        data['data'] = public.M('logs2').where('type=?', (u'gopanel login reminder',)).field('log,addtime').order('id desc').limit(
