...
from gopanel import session,cache,json_header
    ...
    def request_get(self,get):
        ...
        if domain:
            if(public.GetHost().lower() != domain.strip().lower()):
                return 404
                # errorStr = public.ReadFile('./gopanel/templates/' + public.GetConfigValue('template') + '/error2.html')
                # try:
