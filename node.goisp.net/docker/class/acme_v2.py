...
class acme_v2:
    ...
    def update_zip(self, args):
        ...
        import panelTask
        GO_TASK = panelTask.GO_TASK()
        zip_file = path+'/cert.zip'
        result = GO_TASK._zip(path, '.', path+'/cert.zip', '/dev/null', 'zip')
    ...
    def cloud_check_domain(self,domain):
        try:
            result = requests.post('https://www.goisp.net/api/panel/checkDomain',{"domain":domain,"ssl":1}).json()
...
'''
    ...
    def register(self, existing=False):
        if not 'email' in self._config:
            self._config['email'] = 'demo@goisp.net'
        if existing:
    ...
    # 获take the current 正在使用此证书的网站目录
