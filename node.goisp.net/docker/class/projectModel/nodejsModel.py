...
class main(projectBase):
    ...
    def get_run_list(self,get):
        ...
        if not os.path.exists(project_cwd): return public.return_error('The project directory does not exist!')
...
'''.format(nodejs_bin_path = nodejs_bin_path)
    ...
    def install_packages(self,get):
        ...
        if not project_find: return public.return_error('The specified item does not exist!')
        if not os.path.exists(project_find['path']): return public.return_error('The project directory does not exist!')
        package_file = '{}/package.json'.format(project_find['path'])
        if not os.path.exists(package_file): return public.return_error('The package.json configuration file was not found in the project directory!')
        ...
        if not npm_bin and not yarn_bin: 
            return public.return_error('The specified nodejs version does not exist!')
        ...
        return public.return_data(True,'The dependency package is installed successfully!')
    ...
    def update_packages(self,get):
        ...
        if not os.path.exists(package_file): return public.return_error('The package.json configuration file was not found in the project directory!')
        package_lock_file = '{}/package-lock.json'.format(project_find['path'])
        if not os.path.exists(package_lock_file): return public.return_error('Please install the dependency package first!')
        nodejs_version = project_find['project_config']['nodejs_version']
        npm_bin = self.get_npm_bin(nodejs_version)
        if not npm_bin: 
            return public.return_error('The specified nodejs version does not exist!')
        ...
        return public.return_data(True,'Dependent package updated successfully!')
    ...
    def reinstall_packages(self,get):
        ...
        return public.return_data(True,'Dependent package reinstalled successfully!')
    ...
    def install_module(self,get):
        ...
        if os.path.exists(filename): return public.return_error('The specified module has been installed!')
        ...
        if not os.path.exists(filename): return public.return_error('Failed to install the specified module!')
        public.WriteLog(self._log_name,'Node project {}, {} module installation is complete!'.format(get.project_name,mod_name))
        return public.return_data(True,'Successful installation!')
    ...
    def uninstall_module(self,get):
        ...
        mod_name = get.mod_name
        filename = '{}/node_modules/{}/package.json'.format(project_cwd,mod_name)
        if not os.path.exists(filename): return public.return_error('The specified module is not installed!')
        ...
        if os.path.exists(filename): 
            ...
            if result.find('looking for funding') != -1:
                return public.return_error("This module is dependent on other installed modules and cannot be uninstalled!")
            return public.return_error("Unable to uninstall this module!")

        public.WriteLog(self._log_name,'Node project {}, {} module uninstallation completed!'.format(get.project_name,mod_name))
        return public.return_data(True,'Module unloaded successfully!')
    ...
    def upgrade_module(self,get):
        ...
        return public.return_data(True,'Module updated successfully!')
    ...
    def create_project(self,get):
        '''
            ...
            @return dict
        '''
        if not isinstance(get,public.dict_obj): return public.return_error('The parameter type is wrong, need dict obj object')
        ...
        if public.M('sites').where('name=?',(get.project_name,)).count():
            return public.return_error('The specified project name already exists: {}'.format(get.project_name))
        get.project_cwd = get.project_cwd.strip()
        if not os.path.exists(get.project_cwd):
            return public.return_error('The project directory does not exist: {}'.format(get.project_cwd))
        
        # 端口占用检测
        if self.check_port_is_used(get.get('port/port')): 
            return public.return_error('This port is already occupied, please modify your project port, port: {}'.format(get.port))
        
        domains = []
        if get.bind_extranet == 1:
            domains = get.domains
            if not public.is_apache_nginx(): return public.return_error('Please install Nginx or Apache first')
        for domain in domains:
            domain_arr = domain.split(':')
            if public.M('domain').where('name=?',domain_arr[0]).count():
                return public.return_error('Domain name already exists: {}'.format(domain))
        pdata = {
        ...
        return public.return_data(True,'Added project successfully',project_id)
    ...
    def modify_project(self,get):
        '''
            ...
            @return dict
        '''
        if not isinstance(get,public.dict_obj): return public.return_error('The parameter type is wrong, need dict obj')
        if not self.is_install_nodejs(get):
            return public.return_error('Please install nodejs version manager before installing at least one nodejs')
        project_find = self.get_project_find(get.project_name)
        if not project_find:
            return public.return_error('Item does not exist: {}'.format(get.project_name))
        ...
        if hasattr(get,'port'): 
            if int(project_find['project_config']['port']) != int(get.port):
                if self.check_port_is_used(get.get('port/port'),True): 
                    return public.return_error('The port is already occupied, please modify your port, port: {}'.format(get.port))
        ...
        if hasattr(get,'project_script'): 
            if not get.project_script.strip():
                return public.return_error('Start command cannot be empty')
        ...
        if rebuild:
            self.rebuild_project(get.project_name)
        return public.return_data(True,'Modify the project successfully')
    ...
    def remove_project(self,get):
        ...
        project_find = self.get_project_find(get.project_name)
        if not project_find:
            return public.return_error('The specified item does not exist: {}'.format(get.project_name))
        ...
        return public.return_data(True,'Successfully deleted item')
    ...
    def project_remove_domain(self,get):
        ...
        if project_find['project_config']['bind_extranet']:
            if len(project_find['project_config']['domains']) == 1: return public.return_error('At least one domain name is required for the mapped project')
        domain_id = public.M('domain').where('name=? AND pid=?',(domain_arr[0],project_id)).getField('id')
        if not domain_id: 
            return public.return_error('The specified domain name does not exist')
        ...
        self.set_config(get.project_name)
        return public.return_data(True,'Domain name deleted successfully')
    ...
    def bind_extranet(self,get):
        ...
        project_name = get.project_name.strip()
        project_find = self.get_project_find(project_name)
        if not project_find: return public.return_error('Item does not exist')
        if not project_find['project_config']['domains']: return public.return_error('Please add at least one domain name in the [Domain Management] option')
        ...
        return public.return_data(True,'Enable the mapping successfully')
    ...
    def unbind_extranet(self,get):
        ...
        return public.return_data(True,'Disabled successfully')
    ...
    def kill_pids(self,get=None,pids = None):
        ...
        if not pids: return public.return_data(True, 'No process')
        ...
        for i in pids:
            try:
                ...
                pass
        return public.return_data(True, 'The process has all ended')
    ...
    def start_project(self,get):
        ...
        if os.path.exists(package_file):
            ...
            if not os.path.exists(node_modules_path):
                return public.return_error('Please go to the [Module] and click [One-key install] to install the module dependencies!')
            package_info = json.loads(public.readFile(package_file))
...
'''.format(
    ...
    last_env = last_env
        ...
        if not os.path.exists(pid_file):
            ...
            if p.find('[Errno 0]') != -1:
                ...
                return public.return_error('The startup command was intercepted by unknown security software, please check the installation software log')
        ...
        if not pids: 
            if os.path.exists(pid_file): os.remove(pid_file)
            return public.return_error('failed to activate<br>{}'.format(public.GetNumLines(log_file,20)))

        return public.return_data(True, 'Successfully started', pids)
    ...
    def get_project_info(self,get):
        ...
        if not project_info: return public.return_error('The specified item does not exist!')
    ...
    def set_project_listen(self,get):
        ...
        return public.return_data(True,'Set successfully')
