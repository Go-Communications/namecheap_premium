...
class plugin_deployment:
    ...
    def get_icon(self,pinfo):
        path = '/www/server/panel/gopanel/static/img/dep_ico'
        ...
        public.ExecShell("wget -O " + filename + ' http://www.goisp.net' + m_uri + " &")
        return pinfo
    ...
    def GetCloudList(self,get):
        try:
            ...
            if not 'package' in session or not os.path.exists(jsonFile) or hasattr(get,'force'):
                downloadUrl = 'http://www.goisp.net/api/panel/get_deplist'
    ...
    def SetupPackage(self,get):
        ...
        if isDownload:
            ...
            if pinfo['versions'][0]['download']: self.DownloadFile('http://www.goisp.net/api/Pluginother/get_file?fname=' + pinfo['versions'][0]['download'], packageZip)
        ...
        if not pinfo: return public.returnMsg(False,'Cannot find [gopanel Auto Deployment Configuration File] in the installation package')
    ...
    def depTotal(self,id):
        ...
        pdata['pid'] = id;
        p_url = 'http://www.goisp.net/api/pluginother/create_order_okey'
