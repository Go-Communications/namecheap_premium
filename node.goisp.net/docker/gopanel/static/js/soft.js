var soft = {
    ...
    get_list: function(page, type, search) {
        ...
        bt.soft.get_soft_list(page, type, search, function(rdata) {
          ...
          // if (type == 10) {
          //   $("#updata_pro_info").html('<div class="alert alert-danger" style="margin-bottom:15px"><strong>' + lan.soft.bt_developer + '</strong><a class="btn btn-success btn-xs va0" href="https://www.goisp.net" title="' + lan.soft.get_third_party_apps + '" style="margin-left: 8px" target="_blank">' + lan.soft.get_third_party_apps + '</a><input type="file" style="display:none;" accept=".zip,.tar.gz" id="update_zip" multiple="multiple"><button class="btn btn-success btn-xs" onclick="soft.update_zip_open()" style="margin-left:8px">' + lan.soft.import_plug + '</button></div>')
          // } else if (type == 11) {
    ...
    set_soft_tips:function(el,type){
        ...
        if(type == 10){
            ...
            explain.text('Security Reminder: gopanel officially conducted a security audit before the third-party plug-in was put on the shelves, but there may be security risks. Please check it out before using it in the production environment.');
            btn_ground = soft.render_tips_btn(btn_ground,[
                //{title:'免费入驻',href:'https://www.goisp.net/developer/',rel:'noreferrer noopener',target:'_blank',btn:'免费入驻',class:'btn btn-success btn-xs va0',style:"margin-left:10px;"},
                {title:'Get third-party apps',rel:'noreferrer noopener',href:'https://www.goisp.net/bbs/forum-40-1.html',target:'_blank',btn:'Get third-party apps',class:'btn btn-success btn-xs va0 ml15',style:"margin-left:10px;"},
            ...
            btn_ground = soft.render_tips_btn(btn_ground,[
                {title:'免费入驻',href:'https://www.goisp.net/bbs/thread-33063-1-1.html',rel:'noreferrer noopener',target:'_blank',btn:'免费入驻',class:'btn btn-success btn-xs va0',style:"margin-left:10px;"},
            ...
            var set_btn_style = function(res){
              if(!res.status || !res){
                ...
                if(type == 12 && (ltd < 0 && pro >=0)){
                  explain.html('企业版可以免费使用专业版及企业版插件，了解专业版和企业版的区别，请点击<a href="https://www.goisp.net/download/linux.html" target="_blank" class="btlink ml5">查看详情</a>。<a href="https://www.goisp.net/bbs/forum.php?mod=viewthread&tid=50342&page=1&extra=#pid179211" target="_blank" class="btlink ml5">《专业版升级企业版教程》</a>');
        ...
        $.post('/deployment?action=GetList', pdata, function(rdata) {
            ...
            if ($(".onekey-type").attr("class") === undefined) {
                ...
                tbody = '<div class="alert alert-info" style="margin-bottom: 10px;">\
                        <strong class="mr5">gopanel one-click deployment has been launched, and we invite global outstanding projects to settle in (limited to project officials)</strong>\
                        <a class="btn btn-success btn-xs mr5" href="https://www.goisp.net/bbs/thread-33063-1-1.html" target="_blank">Free entry</a>\
            ...
            for (var i = 0; i < rdata.list.length; i++) {
                ...
                zbody += '<tr>' +
                    ...
                    '<td><a class="btlink" target="_blank" href="' + rdata.list[i].official + '">' + (rdata.list[i].author == 'gopanel' ? rdata.list[i].title : rdata.list[i].author) + '</a></td>' +
...
function timeago(dateTimeStamp) { //dateTimeStamp是一个时间毫秒，注意时间戳是秒的形式，在这个毫秒的基础上除以1000，就是十位数的时间戳。13位数的都是时间毫秒。
    ...
    var minute = 1000 * 60,
        ...
        halfamonth = day * 15,
        month = day * 30,
        now = new Date().getTime(), //获take the current 时间毫秒
