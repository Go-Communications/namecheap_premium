...
function toShell(){
  ...
  if($("b[val='toShell']").text() === 'synchronised time'){
    ...
    shell_body = 'echo "|-Trying to synchronize time from 0.pool.goisp.net..";\n\
ntpdate -u 0.pool.goisp.net\n\
if [ $? = 1 ];then\n\
  echo "|-Trying to synchronize time from 1.pool.goisp.net..";\n\
  ntpdate -u 1.pool.goisp.net\n\
fi\n\
if [ $? = 1 ];then\n\
...
if [ $? = 1 ];then\n\
  echo "|-Trying to sync time from www.goisp.net..";\n\
...
function message_channel_start(){
  $.post('/config?action=get_settings2',function(res){
    ...
    if(!res.user_mail.user_name && !res.telegram.setup){
      wBody = '<span style="color:red;">No message channel is set, please go to panel settings to add a message channel configuration<a href="https://www.goisp.net/bbs/thread-42312-1-1.html" target="_blank" class="bt-ico-ask" style="cursor: pointer;">?</a></span>';
