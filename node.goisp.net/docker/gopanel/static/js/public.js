...
var aceEditor = {
    ...
    saveAceConfig: function(data,callback){
		...
		this.saveFileBody({
			path:'/www/server/panel/gopanel/static/ace/ace.editor.config.json',
    ...
    getEditorConfig: function(callback) {
        ...
        this.getFileBody({path:'/www/server/panel/gopanel/static/ace/ace.editor.config.json'},function(rdata){
			layer.close(loadT);
				...
				if(jqXHR.status == 500 && (jqXHR.responseText.indexOf('An error occurred while the panel was running') != -1 || error_find != -1)){
					...
					if(error_find != -1){
						...
						<ul class="help-info-text">\
							...
							<li style="list-style: none;">2、如上述尝试未能解除此错误，请截图此窗口到宝塔论坛发贴寻求帮助, 论坛地址：<a class="btlink" href="https://www.goisp.net/bbs" target="_blank">https://www.goisp.net/bbs</a></li>\
						</ul>\
					</div>'
...
//去关联列表
function GetgopanelList() {
    var con = '';
    $.post("/config?action=GetPanelList", function(rdata) {
        for (var i = 0; i < rdata.length; i++) {
            con += '<h3 class="mypcip mypcipnew" style="opacity:.6" data-url="' + rdata[i].url + '" data-user="' + rdata[i].username + '" data-pw="' + rdata[i].password + '"><span class="f14 cw">' + rdata[i].title + '</span><em class="btedit" onclick="bindgopanel(0,\'c\',\'' + rdata[i].title + '\',\'' + rdata[i].id + '\',\'' + rdata[i].url + '\',\'' + rdata[i].username + '\',\'' + rdata[i].password + '\')"></em></h3>'
        }
        $("#newbtpc").html(con);
        $(".mypcipnew").hover(function() {
            ...
            var loginForm = '<div id="gopanelform" style="display:none"><form id="togopanel" action="' + murl + '/login" method="post" target="btpfrom">\
...
GetgopanelList();
//添加面板快捷登录
function bindgopanel(a, type, ip, btid, url, user, pw) {
    var titleName = lan.bt.panel_add;
    if (type == "b") {
        btn = "<button type='button' class='btn btn-success btn-sm' onclick=\"bindgopanel(1,'b')\">" + lan.public.add + "</button>";
    } else {
        titleName = lan.bt.panel_edit + ip;
        btn = "<button type='button' class='btn btn-default btn-sm' onclick=\"bindgopaneldel('" + btid + "')\">" + lan.public.del + "</button><button type='button' class='btn btn-success btn-sm' onclick=\"bindgopanel(1,'c','" + ip + "','" + btid + "')\" style='margin-left:7px'>" + lan.public.edit + "</button>";
    }
						...
						<div class="plugin_body">\
							...
							<div class="conter_box" style="display:none">\
                                ...
                                <ul class="help-info-text c7" style="margin-top: 315px;">\
                                    ...
                                    <li>e.g: [ 12345677:AAAAAAAAA_a0VUo2jjr__CCCCDDD ] <a class="btlink" href="https://forum.goisp.net/d/5115-how-to-add-telegram-to-panel-notifications" target="_blank" rel="noopener"> Help</a></li>\
                                </ul>\
							</div>\
						</div>\
					</div>\
