...
REQUEST_FORM: {request_form}
  USER_AGENT: {user_agent}'''.format(
    ...
    result = public.readFile(public.get_panel_path() + '/gopanel/templates/default/panel_error.html').format(error_title=error_info.split("\n")[-1],request_info = request_info,error_msg=error_info)
...
def colony_route(module = 'index',action = None):
    ...
    try:
        if act_type == 'json':
            ...
            template_file = 'gopanel/templates/colony/{}'.format(template_name)
...
def send_favicon():
    ...
    s_file = '/www/server/panel/gopanel/static/favicon.ico'
...
def sock_shell(ws):
    '''
        ...
        示例：
            ...
            p.send('ping www.goisp.net -c 100')
    '''
    comReturn = comm.local()
    if comReturn:
...
def close_sock_shell():
    '''
        ...
        @return dict
        示例：
            $.post('/close_sock_shell',{cmdstring:'ping www.goisp.net -c 100'})
    '''
    comReturn = comm.local()
    if comReturn: return comReturn
    args = get_input()
