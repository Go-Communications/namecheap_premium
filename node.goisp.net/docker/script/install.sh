...
+----------------------------------------------------------------------
| Copyright (c) 2015-2017 BT-SOFT(http://www.goisp.net) All rights reserved.
+----------------------------------------------------------------------
| Python2.6/2.7 successful the http://SERVER_IP:8888 is WebPanel
+----------------------------------------------------------------------
"

download_Url='http://download.goisp.net'
setup_patn=/www

while [ "$go" != 'y' ] && [ "$go" != 'n' ]
do
	read -p "Now do you want to install go-panel to the $setup_patn directory?(y/n): " go;
done
...
curl http://www.goisp.net/Api/SetupCount?type=Linux

echo "====================================="
echo -e "\033[32mThe install successful!\033[0m"
echo -e "====================================="
echo -e "go-panel: http://$address:8888"
